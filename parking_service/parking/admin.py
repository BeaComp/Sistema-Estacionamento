...
class ParkingRecordAdmin(admin.ModelAdmin):
    list_display = ['vehicle', 'parking_record', 'entry_time', 'exit_time']
    search_fields = ['vehicle__license_plate', 'parking_record__spot_number']
    
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        if db_field.name == 'parking_record' and not request.resolver_match.url_name.endswith(
            'change'
        ):
